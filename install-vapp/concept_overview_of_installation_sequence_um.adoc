---
permalink: install-vapp/concept_overview_of_installation_sequence_um.html 
sidebar: sidebar 
keywords: overview, installation, sequence 
summary: Im Installations-Workflow werden die Aufgaben beschrieben, die Sie vor der Verwendung von Unified Manager ausführen müssen. 
---
= Informationen zur Installationsreihenfolge von Active IQ Unified Manager
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Im Installations-Workflow werden die Aufgaben beschrieben, die Sie vor der Verwendung von Unified Manager ausführen müssen.

In diesen Abschnitten werden die im folgenden Workflow gezeigten Elemente beschrieben.

image::../media/overall_um_install_flow.png[Ein Diagramm, das die Arbeitsabläufe für die Installation, die Ersteinrichtung und die administrativen Einstellungen von Unified Manager veranschaulicht]
