...
---
= Informationen dazu, wie Active IQ Unified Manager die virtuelle Infrastruktur von VMware überwacht
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image::../media/vm_deployment.gif[Ein Diagramm, das VMware Hosts, VMs, Datenspeicher, VMDKs, IP-Netzwerk und Speicherverbindungen zeigt]

*Benutzer-Workflow*

Das folgende Diagramm zeigt einen typischen Anwendungsfall der VM-Topologieansicht:

image::../media/vm_workflow.gif[Ein Diagramm, das einen Workflow zur Ermittlung der Latenz von VM-Anwendungen in den Schichten Compute, Netzwerk und Storage zeigt]
