...
---
= Hinzufügen von Benachrichtigungen in Active IQ Unified Manager
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Bitte beachten Sie, dass beim Hinzufügen einer Storage Virtual Machine (SVM) oder eines Clusters zu den Ressourcen einer Warnung keine untergeordneten Objekte automatisch hinzugefügt werden. Wenn Sie beispielsweise einen Cluster zu den Ressourcen einer Warnung hinzufügen, werden die Knoten oder LUNs nicht automatisch zur Warnung hinzugefügt.
